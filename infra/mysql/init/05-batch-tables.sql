...
-- ============================================================================
-- settlements: 가맹점 정산 결과
-- settlement Job이 payment_db.payments를 집계 후 INSERT
-- ============================================================================
...
-- ============================================================================
-- discrepancies: 대사 불일치 내역
-- reconciliation Job이 은행 mock 응답 vs 내부 charges 비교 후 INSERT
-- ============================================================================
...
-- ============================================================================
-- settlement_outbox_events: 정산 이벤트 발행 대기 테이블 (Transactional Outbox)
-- Settlement 저장과 동일 트랜잭션에서 INSERT → OutboxPublisher가 polling 후 Kafka 발행
-- ============================================================================
CREATE TABLE IF NOT EXISTS settlement_outbox_events (
    id          BIGINT       NOT NULL AUTO_INCREMENT,
    topic       VARCHAR(100) NOT NULL COMMENT 'Kafka 토픽',
    message_key VARCHAR(128) NOT NULL COMMENT 'Kafka 파티셔닝 키 (externalId)',
